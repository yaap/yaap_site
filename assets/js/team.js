...
async function fetchGitHubUser(username) {
	try {
		const response = await fetch(
			`https://yaaprom-device-info.idoybh2.workers.dev/users/${username}`
		);
...
async function buildMaintainerDeviceMap({
	...
	try {
		...
		try {
			...
			if (!treeResp.ok)
				throw new Error(`GitHub trees request failed ${treeResp.status}`);
		} catch (err) {
		...
		const jsonPaths = treeJson.tree
			.filter(
				item => item.path && item.type === "blob" && item.path.endsWith(".json")
			)
			.map(item => item.path);
		...
		const fetched = await mapWithConcurrencyLimit(
			jsonPaths,
			async path => {
				try {
		...
		for (let i = 0; i < jsonPaths.length; i++) {
			...
			const codename =
				path.split("/")[0] || path.replace(".json", "").split("/").pop();
		...
		try {
			localStorage.setItem(
				cacheKey,
				JSON.stringify({ ts: Date.now(), data: maintainerDevices })
			);
		} catch (e) {}
...
async function buildMaintainerDeviceMapFallback({
	useCache = true,
	cacheTTLms = 1000 * 60 * 5
} = {}) {
	if (deviceCache) return deviceCache;
	try {
		...
		const response = await fetch(
			"https://yaaprom-device-info.idoybh2.workers.dev/repos/yaap/device-info/contents/"
		);
					...
					.map(async folder => {
						try {
							const folderResponse = await fetch(
								`https://yaaprom-device-info.idoybh2.workers.dev/repos/yaap/device-info/contents/${folder.name}`
							);
							if (!folderResponse.ok) return;
							const folderContents = await folderResponse.json();
							const jsonFiles = folderContents.filter(
							...
							await Promise.all(
								jsonFiles.map(async jsonFile => {
									try {
										...
										if (deviceInfo.maintainer) {
											const maintainers = parseMaintainers(
												deviceInfo.maintainer
											);
											maintainers.forEach(maintainer => {
												if (!maintainerDevices[maintainer])
													maintainerDevices[maintainer] = [];
...
function createDeviceBadges(devices, maintainerIndex) {
	...
	let html = visibleDevices
		.map(
			device =>
				`<a href="/device.html?codename=${encodeURIComponent(
					device.codename
				)}" class="device-badge" title="${escapeHtml(
					device.name || device.device || device.codename
				)}">
	...
	if (hiddenDevices.length > 0) {
      ...
      <button class="device-toggle-btn" onclick="toggleDevices(${maintainerIndex}, ${
...
function toggleDevices(maintainerIndex, hiddenCount) {
	const hiddenDiv = document.getElementById(
		`hidden-devices-${maintainerIndex}`
	);
	const toggleBtn = document.querySelector(
		`button[onclick="toggleDevices(${maintainerIndex}, ${hiddenCount})"]`
	);
...
function createTeamCard(member, githubData) {
	const name =
		(githubData && (githubData.name || githubData.login)) || member.github;
    ...
    <div class="team-card">
      <img src="${avatar}" alt="${escapeHtml(
				name
			)}" class="team-card__photo" loading="lazy" />
      ...
      <div class="team-card__links">
        <a href="https://github.com/${encodeURIComponent(
					member.github
				)}" class="social-link github" target="_blank" aria-label="GitHub">
          ${icons.github}
        </a>
        <a href="https://t.me/${encodeURIComponent(
					member.telegram
				)}" class="social-link telegram" target="_blank" aria-label="Telegram">
          ${icons.telegram}
        </a>
      </div>
    </div>
...
function createMaintainerCard(maintainer, githubData, devices, index) {
	const name =
		(githubData && (githubData.name || githubData.login)) || maintainer;
	const bio =
		(githubData && githubData.bio) ||
		`Maintains ${devices.length} device${
			devices.length > 1 ? "s" : ""
		} for YAAP`;
	const avatar =
		(githubData && githubData.avatar_url) ||
		`https://github.com/${maintainer}.png`;
	const telegramId =
		(devices.find(d => d.tg_id) && devices.find(d => d.tg_id).tg_id) ||
		maintainer;
  ...
  `
    ...
    <div class="maintainer-card">
      <img src="${avatar}" alt="${escapeHtml(
				name
			)}" class="maintainer-card__photo" loading="lazy" />
...
async function loadTeamMembers({
	teamSelector = "#team-grid",
	maintainersSelector = "#maintainers-grid",
	showWebsiteTeam = true,
	showDesignTeam = true
} = {}) {
	...
	if (teamGrid) {
		teamGrid.innerHTML = teamMembers
			.map(() => '<div class="team-card loading"></div>')
			.join("");
	}
	...
	async function fetchProfiles(members) {
		...
		const workers = Array(Math.min(limit, members.length))
			.fill()
			.map(() => worker());
		await Promise.all(workers);
		return results;
	...
	if (document.querySelector("#website-grid") && websiteTeam.length) {
		...
		grid.innerHTML = websiteTeam
			.map((m, i) => createTeamCard(m, websiteProfiles[i]))
			.join("");
	}

	if (document.querySelector("#design-grid") && designTeam.length) {
		const grid = document.querySelector("#design-grid");
		grid.innerHTML = designTeam
			.map((m, i) => createTeamCard(m, designProfiles[i]))
			.join("");
	}
	...
	if (maintainersGrid) {
		...
		const maintainers = Object.keys(maintainerMap).sort((a, b) =>
			a.localeCompare(b, undefined, { sensitivity: "base" })
		);
		maintainersGrid.innerHTML = maintainers
			.map(() => '<div class="maintainer-card loading"></div>')
			.join("");
		...
		const workers = Array(Math.min(limit, maintainers.length))
			.fill()
			.map(() => workerRender());
		await Promise.all(workers);
	}
